...
$out = trim($ssh->exec('sudo /sbin/containermanager diskgb ' . trim($ctid)));

if (strpos($out, 'Halting.')) {
return 'N/A';
} else {
return $out;
}
...
$out = trim($ssh->exec('sudo /sbin/containermanager rammb ' . trim($ctid)));
...
$create = trim($ssh->exec('/usr/bin/sudo /sbin/containermanager create '.$ctid.' '.$formattedos));
