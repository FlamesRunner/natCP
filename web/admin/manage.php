...
if (!$ath->rowCount() > 0) {
header('Location: /admin/listvps.php');
...
} elseif ($_GET['act'] == "destroy") {
$destroy = destroyContainer($_GET['vserver']);
$rmquery = $dbh->prepare('DELETE from virtualservers where ctid=:ctid');
$rmquery->bindParam(':ctid', $_GET['vserver']);
$rmquery->execute();

header('Location: /admin/listvps.php');
die('Container destroyed');
...
<br /><br />
<a href="manage.php?vserver=<?php echo $_GET['vserver']; ?>&act=destroy" class="btn btn-danger" onclick="loading();">Destroy container</a>
</div>
</div>
